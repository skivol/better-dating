...
import { libraries as serverLibraries, tools as serverTools, infrastructure as serverInfrastructure } from './acknowledgements/backend';
...
const License = ({ license, text }: LicenseProps) => (
    <Paper id={licenseId(license)} elevation={3}
...
const AcknowledgementEntry = ({ name, url, license, copyright }: AcknowledgementEntryProps) => {
    ...
    return (
        <li>
            ...
            {copyrights && copyrights.map((c: any) => <Typography key={c}>{c}</Typography>)}
        </li>
    );
};
...
const AcknowledgementEntries = ({ entries }: any) => (
    <ul>
        {entries.sort((a: any, b: any) => a.name > b.name ? 1 : -1).map((library: any) => (
        ...
        <>
            ...
            >
                <Paper elevation={3} className="u-padding-15px">
                    ...
                    <Typography className="u-margin-bottom-10px" variant="h4">{Messages.licensesText}</Typography>
